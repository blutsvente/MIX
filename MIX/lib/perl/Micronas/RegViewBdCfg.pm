...
sub _gen_view_bdcfg {
    ...
    $this->global(
				  ...
				  'file_stl'           => $eh->get("reg_shell.bd-cfg.stl_file_name"),
				  'header'             => "
...
"
    ...
    # make it so
    $this->_write_backdoor_config($lref_domains, \%hpaths, \@lbsi, $OPTVAL{'bsi'});
    
	return 1;
};

sub _write_backdoor_config {
    my ($this, $lref_domain_names, $href_hpaths, $lref_bsi, $source_str) = @_;
    ...
    my $dest_stl = new FileHandle $eh->get("output.path") . "/" . $this->global->{"file_stl"} , "w";

    print $dest_nc join("\n# ", split(/\n/, $this->global->{"header"}), "bsi source file: $source_str") . "\n";
    print $dest_axis join("\n\/\/ ", split(/\n/, $this->global->{"header"}), "bsi source file: $source_str") . "\n";
    print $dest_stl "
# Socket Transaction Language file
# automatically generated by MIX (module RegViewBdCfg.pm)
# bsi source file: $source_str
version 2.0
...
";
    ...
    foreach my $domainname (sort keys %hregs_in_bsi) {
        ...
        foreach my $regname (sort keys %{$hregs_in_bsi{$domainname}}) {
            ...
            print $dest_nc $this->_get_nc_force_frc($o_domain, $o_reg, $reg_offset, $href_hpaths);
            print $dest_axis $this->_get_axis_force_frc($o_domain, $o_reg, $reg_offset, $href_hpaths);
        };
    };
    ...
    # dump all to an STL file
    my %hregs_by_absolute_address = ();
    foreach my $domainname (sort keys %hregs_in_bsi) {
        $o_domain = $this->find_domain_by_name_first($domainname);
        ...
        if ($bank_no =~ m/(\d+)$/) {  
            ...
            foreach my $regname (sort keys %{$hregs_in_bsi{$domainname}}) {
                $o_reg = $hregs_in_bsi{$domainname}->{$regname};
                $reg_offset = $o_domain->get_reg_address($o_reg);
                $hregs_by_absolute_address{$reg_offset + ($bank_no << 8)} =  $o_reg;
            };
        } else {
...
sub _get_nc_force_frc {
    my ($this, $o_domain, $o_reg, $offset, $href_hpaths) = @_;
    ...
    if ($bank_no =~ m/(\d+)$/) {
        ...
        if (exists $href_hpaths->{$bank_no}) {
            my $value = $o_reg->get_reg_init();
            my $hpath = $href_hpaths->{$bank_no};
            my $str = "# register ".($o_reg->name)."\n";
            # note: the hex literals are printed in VHDL syntax, this is okay for frc project
            $str .= join(" ",
                           $eh->get("reg_shell.bd-cfg.ncsim_cmd"),
                           $hpath . ".write_reg_" . sprintf("%x", $offset) . ".reg_data_out_int",
                           "{X\""._val2hex($eh->get("reg_shell.datawidth"), $value)."\"};"
                          ) . "\n";
            $str .= join(" ",
                         $eh->get("reg_shell.bd-cfg.ncsim_cmd"),
