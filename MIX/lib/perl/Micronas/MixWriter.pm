...
sub gen_instmap ($;$$) {
    ...
    if ( $lang =~ m,^veri,io and $eh->get( 'output.generate.emumux.modules' ) ) {
    	( $map, $emumux ) = _mix_wr_insert_emumux( $inst, $map, \@in, \@out, $tcom );
    	# $map = $emumux . $map;
    }
    ...
    #!wig20070426: emumux outside of ifdef (verimap)
    $map = ( $emumux . $map ) if $emumux;
...
sub _mix_wr_insert_emumux ($$$$$) {
	...
	my $matchmodules = '(' . join( '|', split( /[,\s+]/, $emudefs->{'modules'} ) ) . ')';
	...
	if ( $emudefs->{'_hiersheet_'} or $inst =~ m/^$matchmodules$/ ) {
		...
		if ( $emudefs->{sigselect} ) { # Take only these signals
			$matchsignals = '(' . join( '|', split( /[,\s+]/, $emudefs->{'sigselect'} ) ) . ')';
		} else {
		...
		# Find all out signals
		my $matchsignals_out = {};     # output signals require different wiring ..
		for my $i ( @$outr ) {
			$matchsignals_out->{$i} = 1;
		}
		$emudefs->{'isout'} = $matchsignals_out; 
		...
		$muxblock =  '%S%' x 1 . '`ifdef ' . $definetag . "\n";
		$muxblock .= '%S%' x 2 . $tcom . ' Emulator Data Injection Path, generated by MIX' . "\n";
		
		# Set (or not) select signal default
		my $selectlevel = " = 1'b0;"; # Default is low
		if ( $emudefs->{'options'} =~ m/\bselect\s*=\s*(.+)/ ) {
			( $selectlevel = $1 ) =~ s/[\s,].*//; # Strip away trailing stuff
			if ( $selectlevel =~ m/^(high|1|on)/i ) {
				$selectlevel = " = 1'b1;";
			} elsif ( $selectlevel =~ m/^(low|0|off)/i ) {
				$selectlevel = " = 1'b0;";
			} elsif ( $selectlevel =~ m/^(open|nc)/i ) {
				$selectlevel = ";";
			} else {
				# Take whatever we got from the user
				$selectlevel = " = " . $selectlevel . ";";
			}
		}
		$muxblock .= '%S%' x 2 . 'wire ' . $selectsig . $selectlevel . "\n";

		# Go through map and rework all matching signal names
		for my $l ( split( /\n/, $map ) ) {
			# Splice comments into seperate string:
			my $lcom = '';
			if ( $l =~ m/$tcom/ ) {
				( $l , $lcom ) = split( /$tcom/, $l );
				$lcom = $tcom . $lcom;
...
sub _insert_emumux_signal ($$$$$$) {
	...
	# Out signals ->
	if ( exists( $emudefs->{isout}->{$signalname} ) ) {
		$extraassign = '%S%' x 2 . 'assign ' . $signalname . $s_sigsplice .
			'%S%= ' . $selectname . ' ? ' .
			$s_ext2mux . '%S%: '. $s_mux2inst . ';' . "\n";
	} else {
