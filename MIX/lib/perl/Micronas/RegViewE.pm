###############################################################################
#  RCSId: $Id: RegViewE.pm,v 1.16 2006/11/13 17:24:42 lutscher Exp $
###############################################################################
#                                  
...
sub _gen_view_vr_ad {
	...
	my $my_reg_file_name = uc(join("_",$this->global->{regfile_prefix}, map {$_->{name}} @ldomains));
	...
	foreach $o_domain (@ldomains) {
		...
		foreach $o_reg (@{$o_domain->regs}) {
			...
			if ($o_reg->{'definition'} eq '') {
			    ...
			    print E_FILE $this->global->{reg_macro}, " ", uc($o_reg->name);
			    print E_FILE " ", $this->global->{regfile_prefix}, "_";
			};
			$ii =0;
            ...
            foreach $singlefield (@thefields_and_theholes) {
                if ($o_reg->{'definition'} eq '') {
                    if ($ii == 0) {
                        print E_FILE uc($singlefield->{parent_block}), " 0x", _val2hex($eh->get('reg_shell.addrwidth'), $reg_offset), " {\n";
                    };
...
.
                    ...
                    if ($singlefield->{pos} == 0){
                        ...
                        $def{$o_reg->{'definition'}}{list}[$aa]{MY_REG_FILE} =  $this->global->{regfile_prefix} . "_" . uc($singlefield->{parent_block});
                        $def{$o_reg->{'definition'}}{list}[$aa]{address} = "0x" . _val2hex($eh->get('reg_shell.addrwidth'), $reg_offset);
                    };
                }; 
            };
            if ($o_reg->{'definition'} eq '') {
                print E_FILE "};\n"; 
                $this->write_extend_coverage(uc($o_reg->name));
            };
        };
    };
    ...
    foreach $cc (keys %def){
        if ($cc ne ''){
            # output: regdef
            print E_FILE $this->global->{reg_macro}, " ", $def{$cc}{list}[0]{MY_REG}, " {\n";
            for $dd(0..$#{ $def{$cc}{fields} } ) {
                printf E_FILE ("   %-7s %-33s : uint(bits:%2s) ", $this->global->{field_macro},$def{$cc}{fields}[$dd]{name}, $def{$cc}{fields}[$dd]{size});
...
sub write_extend_coverage {
    ...
    if ($this->global->{'vplan_ref'} !~ m/%empty%/i) {
        print E_FILE "extend ",uc($name), " vr_ad_reg {\n";
        print E_FILE "  cover reg_access (kind == ",uc($name),") using also vplan_ref = \"", $this->global->{'vplan_ref'},"\";\n";
        print E_FILE "};\n\n";
    };
};
