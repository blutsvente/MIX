#
# $Id: MarkedDrv.pm,v 1.1 2003/02/03 13:18:28 wig Exp $
#
...
# HISTORY
# $Log: MarkedDrv.pm,v $
# Revision 1.1  2003/02/03 13:18:28  wig
# Initial revision
#
# Revision 1.2  2001/07/23 16:23:30  jentzsch
# Output behaviour changed
#
# Revision 1.1  2001/07/19 16:50:35  jentzsch
# New
#
# Revision 1.2  2001/07/06 11:57:07  armitage
# Added Error: prefix to all logerr messages by default.
#
...
sub map_pri {
        my $self = shift;
        my ($priority, $level) = @_;
	my @prio = qw(EMERG ALERT CRITICAL ERROR WARNING WARNING NOTE NOTE INFO INFO DEBUG);
	#return uc($priority);              # ignored for basic drivers
	return sprintf("%-9s", $prio[$level]);              # ignored for basic drivers
}
...
sub write {
	...
	local $\ = undef;
	print main::STDERR $priority.$logstring."\n";
}
...
sub logconfess {
	...
	die $self->map_pri("", 0).$self->prefix_msg("$msg\n");
}
...
sub logxcroak {
	...
	die $self->map_pri("", 1).$self->prefix_msg("$msg\n");
}
...
sub logdie {
	my $self = shift;
	my ($str) = @_;
	die $self->map_pri("", 2).$self->prefix_msg("$str\n");
}
...
sub logerr {
	...
	my ($str) = @_;
	#$str->prepend("ERROR: ");
	warn $self->map_pri("", 3).$self->prefix_msg("$str\n");
}
...
sub logwarn {
	...
	my ($str) = @_;
	#$str->prepend("WARNING: ");
	warn $self->map_pri("", 4).$self->prefix_msg("$str\n");
}
...
sub logxcarp {
	...
	require Carp;
	my $msg = $self->carpmess($offset, $str, \&Carp::shortmess);
	warn $self->map_pri("", 4).$self->prefix_msg("$msg\n");
}
...
Log::Agent::Driver::Marked - logging driver for Log::Agent with marked output
